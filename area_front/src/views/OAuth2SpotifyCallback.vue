<template>
  ...
  <div v-if="Object.keys(callbackParams).length" class="debug-section">
    ...
    <p class="debug-description">Copy these values when checking the Spotify redirect.</p>
    <ul class="debug-list">
...
const subtitle = ref('Processing your Spotify login...')
...
onMounted(async () => {
  try {
    ...
    if (stateParam === 'link') {
      subtitle.value = 'Linking your Spotify account...'
      message.value = 'Linking your Spotify account...'

      const result = await linkSpotifyAccount(code)

      successTitle.value = 'Spotify Account Linked!'
      message.value = result?.spotify_email
        ? `Spotify account linked (${result.spotify_email})`
    ...
    success.value = true
    loading.value = false
    message.value = 'Spotify login successful!'
    ...
    loading.value = false
    error.value = err instanceof Error ? err.message : 'Failed to authenticate with Spotify'
    console.error('Spotify OAuth2 error:', err)
  }
})
</script>
