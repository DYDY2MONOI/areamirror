    ...
    <div class="callback-container">
      <div class="callback-card">
        <div class="callback-header">
          ...
          <p class="callback-subtitle">Processing your Facebook login...</p>
        </div>
...
const error = ref('')
const message = ref('Connecting to Facebook...')
...
onMounted(async () => {
  try {
    ...
    message.value = 'Authenticating with Facebook...'

    const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/oauth2/facebook/callback?code=${code}`)

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({ error: 'Authentication failed' }))
      throw new Error(errorData.error || 'Facebook authentication failed')
    }
    ...
    success.value = true
    loading.value = false
    message.value = 'Facebook login successful!'
    ...
    loading.value = false
    error.value = err instanceof Error ? err.message : 'Failed to authenticate with Facebook'
    console.error('Facebook OAuth2 error:', err)
  }
})
</script>
