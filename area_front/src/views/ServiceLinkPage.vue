        ...
        >
          <div class="service-header">
            <div class="service-icon" :style="{ backgroundColor: service.color }">
              ...
              <svg v-else-if="service.icon === 'amazon'" width="32" height="32" viewBox="0 0 24 24" fill="currentColor">
...
const { currentUser, linkGitHubAccount, unlinkGitHubAccount, linkGoogleAccount, unlinkGoogleAccount, linkAmazonAccount, unlinkAmazonAccount } = useAuth()
...
const linkedServicesCount = computed(() => {
  ...
  if (currentUser.value.amazon_id) count++
  ...
  try {
    ...
    if (serviceId === 'github') {
      ...
      if (!googleClientId || googleClientId === 'your_google_client_id_here') {
        errorMessages.value[serviceId] = 'Google OAuth not configured. Please set the VITE_GOOGLE_CLIENT_ID environment variable.'
        return
      }
      ...
      if (!amazonClientId || amazonClientId === 'your_amazon_client_id_here') {
        errorMessages.value[serviceId] = 'Amazon OAuth not configured. Please set the VITE_AMAZON_CLIENT_ID environment variable.'
        return
      }
...
const unlinkService = async (serviceId: string) => {
  ...
  try {
    if (serviceId === 'github') {
      ...
      successMessages.value[serviceId] = 'Amazon account unlinked successfully'
    } else {
...
const handleServiceCallback = async (serviceId: string, code: string) => {
  ...
  try {
    if (serviceId === 'github') {
      ...
      successMessages.value[serviceId] = 'Amazon account linked successfully!'
    } else {
