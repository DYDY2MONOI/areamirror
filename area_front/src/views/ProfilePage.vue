              ...
              >
                <div class="account-header">
                  <div class="account-icon" :style="{ backgroundColor: service.color }">
                    ...
                    <svg v-else-if="service.icon === 'amazon'" width="24" height="24" viewBox="0 0 24 24" fill="currentColor">
                      <path d="M12 2l6 16h-2.3l-1.3-3.8H9.6L8.3 18H6L12 2zm0 6.4L10.3 13h3.4L12 8.4z"/>
                      <path d="M4.5 19.8c2.5 1.6 12.5 1.6 15 0 .4-.2.9.1.7.6-.9 1.9-5 3-8.7 3s-7.8-1.1-8.7-3c-.2-.5.3-.8.7-.6z"/>
                    </svg>
...
const { currentUser, isAuthenticated, linkGitHubAccount, unlinkGitHubAccount, linkGoogleAccount, unlinkGoogleAccount, linkFacebookAccount, unlinkFacebookAccount, linkAmazonAccount, unlinkAmazonAccount, uploadProfileImage, getProfileImageUrl, refreshProfile } = useAuth()
...
const isServiceLinked = (serviceId: string): boolean => {
  ...
  switch (serviceId) {
    ...
    case 'amazon':
      return !!currentUser.value.amazon_id
    case 'facebook':
...
const linkService = async (serviceId: string) => {
  ...
  try {
    ...
    } else if (serviceId === 'amazon') {
      const amazonClientId = import.meta.env.VITE_AMAZON_CLIENT_ID || 'your_amazon_client_id_here'

      if (!amazonClientId || amazonClientId === 'your_amazon_client_id_here') {
        errorMessages.value = { ...errorMessages.value, [serviceId]: 'Amazon OAuth not configured. Please set the VITE_AMAZON_CLIENT_ID environment variable.' }
        return
      }

      const redirectUri = encodeURIComponent(`${window.location.origin}${service.callbackPath}`)
      const scopeParam = encodeURIComponent(service.scopes.join(' '))
      const amazonAuthUrl = `${service.authUrl}?client_id=${amazonClientId}&redirect_uri=${redirectUri}&scope=${scopeParam}&response_type=code`
...
const unlinkService = async (serviceId: string) => {
  ...
  try {
    if (serviceId === 'github') {
      ...
      successMessages.value = { ...successMessages.value, [serviceId]: 'Amazon account unlinked successfully' }
    } else if (serviceId === 'facebook') {
...
const handleServiceCallback = async (serviceId: string, code: string) => {
  ...
  try {
    if (serviceId === 'github') {
      ...
      const result = await linkAmazonAccount(code)
      successMessages.value = { ...successMessages.value, [serviceId]: 'Amazon account linked successfully!' }
    } else if (serviceId === 'facebook') {
